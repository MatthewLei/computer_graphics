...
//    program = InitShader( "vshader.glsl", "fshader.glsl" );
    ...
    for (int i = 0; i < objs.size(); i++) {
        ...
        GLuint manip_color = glGetAttribLocation( program, "solid_color" ); //called solid_color in vshader
...
keyboard( unsigned char key, int x, int y )
{
    switch( key ) {
        ...
        case ' ':
            if (toon_mode == 1) {
                int input;
                promptInteger("Enter a number higher than 2: ", &input);
                num_shades = input;
                glUniform1i(glGetUniformLocation( program, "num_shades"), num_shades);
            }
            glutPostRedisplay();
...
void myMouse(int button, int state, int x, int y)
{
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        ...
        for (int i = 0; i < objs.size(); i++) {
            ...
            GLuint objectColorId = glGetUniformLocation(program, "object_id_color");
